...
function WorkBlockTemplateCard() {
  return (
    ...
    >
      <span className="text-sm font-medium text-gray-800">Work Block</span>
        ...
        <div ref={draggableContainerRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
          {mode === 'work_blocks' ? (
            <>
              ...
              {/* Deep Work (AIM Block) section — always shown */}
              <div>
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[10px] text-gray-400 ml-auto">
                    {unscheduledItems.filter((i) => i.itemType === 'aim').length}
                  </span>
                </div>
                {unscheduledItems.filter((i) => i.itemType === 'aim').length === 0 ? (
                  <p className="text-xs text-gray-400 px-1 py-2">No AIM instances this week</p>
                ) : (
                  unscheduledItems
                    .filter((i) => i.itemType === 'aim')
                    .map((item) => (
