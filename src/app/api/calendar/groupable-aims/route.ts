 ...
 *  - Have not been dismissed by the requesting user
 *  - Have not already been attended (own instance same category+time exists)
 *
...
export async function GET(request: NextRequest) {
  ...
  const result = visibleInstances
    .filter((inst) => {
      // Skip instances the user is already attending (own instance same category+date)
      const key = `${inst.aimCategoryId}:${inst.scheduledDate.toISOString()}`;
      return !ownCategoryDateSet.has(key);
    })
    .map((inst) => {
      const dismissal = inst.dismissals[0];
      let attendStatus: 'NONE' | 'GOING' | 'MAYBE' | 'NOT_GOING' = 'NONE';
      if (dismissal?.status === 'MAYBE') attendStatus = 'MAYBE';
