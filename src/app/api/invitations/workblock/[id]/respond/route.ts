...
export async function POST(
  ...
  const invitation = await prisma.workBlockInvitation.findUnique({ where: { id } });
  if (!invitation) return notFoundResponse('WorkBlockInvitation');
  ...
  const updated = await prisma.workBlockInvitation.update({
    where: { id },
    data: {
