...
/**
 * Find every recurring-series master / one-off whose title and description
 * unambiguously identify it as a Prism-authored event but which lacks the
 * `prismManaged='1'` extended-property tag — i.e. legacy orphans created
 * before the tagging system shipped. The tag-based sweep cannot see them,
 * the title-only Pass B can't safely delete them (no `prismManaged` filter),
 * and they only show up at run time as duplicate recurring instances.
 *
 * Identification is intentionally narrow:
 *   - exact title match against one of `titles` (Prism's hardcoded set)
 *   - description starts with `"Start your "` AND contains `" in Prism:"`
 *
 * That description is generated by `buildPowerdownSeriesConfig` /
 * `buildReviewSeriesConfigs` in the sync route — a user-authored event
 * matching both clauses verbatim is implausible.
 *
 * Uses `singleEvents: false` so we get MASTERS, not expanded instances —
 * deleting an instance only cancels one occurrence; deleting the master
 * removes the entire series. Skips modified-instance overrides.
 *
 * Per-calendar failures are isolated.
 */
export async function listUntaggedPrismLookalikes(
  userId: string,
  calendarIds: string[],
  ...
  const calendar = await getCalendarClient(userId);
  if (!calendar) return [];
  if (calendarIds.length === 0 || titles.length === 0) return [];

  const results = await Promise.all(
    calendarIds.flatMap((calendarId) =>
      titles.map(async (title) => {
        try {
          const out: (Record<string, unknown> & { id: string; _sourceCalendarId: string })[] = [];
          let pageToken: string | undefined;
          do {
            const response = await withBackoff(
              () =>
              ...
              `events.list(untagged-lookalikes) ${calendarId} ${title}`,
            );
            for (const ev of response.data.items ?? []) {
              if (!ev.id) continue;
              if (ev.status === 'cancelled') continue;
              // `q` is fuzzy; tighten to exact title.
              if (ev.summary !== title) continue;
              // Skip modified-instance overrides; their master is handled separately.
              if (ev.recurringEventId) continue;
              // Already tagged → handled by tag-based Pass A.
              if (ev.extendedProperties?.private?.prismManaged === '1') continue;
              // Require Prism's exact description shape so we never delete a
              // user-authored event with a coincidentally identical title.
              const desc = typeof ev.description === 'string' ? ev.description : '';
              if (!desc.startsWith('Start your ') || !desc.includes(' in Prism:')) continue;
              out.push({ ...ev, id: ev.id, _sourceCalendarId: calendarId });
            }
            pageToken = response.data.nextPageToken ?? undefined;
          } while (pageToken);
          return out;
          ...
          console.error(
            `[calendar] listUntaggedPrismLookalikes failed for ${calendarId} title="${title}":`,
            err,
          );
          return [];
        }
