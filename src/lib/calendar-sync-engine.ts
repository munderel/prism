...
export interface CalendarSyncResult {
  ...
  swept?: Array<{ id: string; summary: string | null; sourceCalendarId: string; reason: string }>;
  googleEventCount?: number;
  oneOffTasksChecked?: number;
  oneOffAimsChecked?: number;
  recurringReviewSeries?: number;
  recurringProcessSeries?: number;
}

/**
 * Reusable per-user Google Calendar sync engine (Issue 8). Drives a full
 * windowed 2-way reconcile + orphan sweep for ONE user and returns a plain
 * result (no HTTP concerns). Used by the POST handler (manual/UI sync) and by
 * the background cron at /api/cron/google-sync.
 *
 * - Serialized per-user by a self-expiring claim lock (`googleSyncLockedAt`):
 *   a manual run and a cron run can't race; stale locks auto-expire in 10 min.
 * - When `viaCron`, an incremental syncToken gate lets the run skip a user with
 *   no Google-side changes (and no unsynced local rows), keeping the 15-min
 *   cron cheap at scale. Manual runs always do the full reconcile.
 */
export async function runCalendarSync(
  userId: string,
  opts: { start: string; end: string; force?: boolean; viaCron?: boolean },
): Promise<CalendarSyncResult> {
  const auth = { userId };
  const start = opts.start;
  const end = opts.end;
  const force = opts.force ?? false;
  const viaCron = opts.viaCron ?? false;

  const rangeStart = new Date(start);
  const rangeEnd = new Date(end);

  // Self-expiring claim lock: serialize background-cron vs manual sync per user
  // WITHOUT holding a DB transaction across slow Google I/O (which would tie up
  // a pooled connection / risk an idle-in-transaction timeout). A crashed run's
  // lock auto-expires after 10 minutes.
  const LOCK_STALE_MS = 10 * 60 * 1000;
  const lockCutoff = new Date(Date.now() - LOCK_STALE_MS);
  const claimed = await conditionalUpdate(prisma.user, {
    where: {
      id: userId,
      OR: [{ googleSyncLockedAt: null }, { googleSyncLockedAt: { lt: lockCutoff } }],
    },
    data: { googleSyncLockedAt: new Date() },
  });
  if (!claimed) {
    return { synced: false, skipped: true, reason: 'sync-in-progress' };
  }

  try {
    return await runCalendarSyncInner();
  } finally {
    await prisma.user.update({ where: { id: userId }, data: { googleSyncLockedAt: null } }).catch(() => {});
  }

  async function runCalendarSyncInner(): Promise<CalendarSyncResult> {
  const user = await prisma.user.findUnique({
    ...
    select: {
      ...
      googleSyncTokenByCalendar: true,
    },
  });

  if (!user?.googleRefreshToken) {
    return { synced: false, error: 'Google Calendar is not connected.' };
  }
  ...
  // Incremental change-gate (cron only). Probe each calendar with its stored
  // syncToken; if nothing changed on Google AND there are no unsynced local
  // rows waiting to push, skip the expensive reconcile. Tokens are always
  // advanced so the next run only sees newer changes. Manual runs skip the gate
  // and always do the full reconcile. Errors are conservative (sync anyway).
  if (viaCron && !force) {
    const tokens = parseSyncTokens(user.googleSyncTokenByCalendar);
    let anyChange = false;
    let hadAllTokens = true;
    for (const calId of calendarIds) {
      const hadToken = !!tokens[calId];
      const probe = await getCalendarChanges(auth.userId, calId, tokens[calId], { seedTimeMin: start });
      if (!hadToken) hadAllTokens = false; // first run for this calendar → must seed
      if (!probe.ok) { anyChange = true; continue; } // couldn't probe → be safe
      if (probe.expired) { anyChange = true; delete tokens[calId]; continue; }
      if (probe.changed) anyChange = true;
      if (probe.nextSyncToken) tokens[calId] = probe.nextSyncToken;
    }
    // Persist advanced cursors whether we skip or run.
    await prisma.user.update({
      where: { id: auth.userId },
      data: { googleSyncTokenByCalendar: tokens as Prisma.InputJsonValue },
    });

    if (!anyChange && hadAllTokens) {
      const [unsyncedTasks, unsyncedAims, unsyncedFood] = await Promise.all([
        prisma.task.count({
          where: {
        ...
        prisma.foodBlock.count({
          where: { userId: auth.userId, calendarEventId: null, startAt: { gte: rangeStart, lte: rangeEnd } },
        }),
      ]);
      if (unsyncedTasks + unsyncedAims + unsyncedFood === 0) {
        return { synced: true, skipped: true, reason: 'no-changes' };
      }
    }
  }
...

  {
    ...
    for (const sr of sweepResults) {
      ...
      // Trace every deletion so a swept event is recoverable from the audit log
      // (log-then-delete — fire-and-forget, never blocks the sweep).
      void writeAuditLog({
        actorId: auth.userId,
        action: 'calendar.orphan_delete',
        targetType: 'GoogleEvent',
        targetId: sr.id,
        metadata: { calendarId: sr.sourceCal, reason: sr.reason, summary: sr.summary ?? null },
      });
    }
  }
